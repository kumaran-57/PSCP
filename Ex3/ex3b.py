...
def main():
    rows = int(input("Number of rows of matrices to be subtracted : "))
    cols = int(input("Number of columns matrices to be subtracted : "))
    first = read_matrix(rows, cols, "first")
    second = read_matrix(rows, cols, "second")

    print("Difference of entered matrices :")
    for i in range(rows):
